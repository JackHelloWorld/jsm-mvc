...
<div class="form-inline">
		<div class="form-group">
			...
			<select type="text" id="type" class="form-control">
				...
				<option value="0">剧本分类</option>
				<option value="1">题材/类型</option>
			</select>
		</div>
		<div class="form-group">
...
var edit = function(){
	...
	if(vas.length<1){
		layer.alert("请选择需要修改的数据",{icon:7});
		return;
	}
	
	$("#parentId").initSelect("${path}/drama_type/find_all",{},"id","name","选择父类型","0",function(d){
		$("#editModel input").val("");
		$("#editModel select").val("0");
		var id = vas[0].id;
		post("${path}/drama_type/find_id",{id:id},function(data){
			...
			layer.open({
				...
				yes : function(index, layero) {
					$("#editForm").sub("${path}/drama_type/update",function(data){
...
var add = function(){
	...
	layer.open({
		...
		btn : [ '保存', '取消' ],
		yes : function(index, layero) {
			$("#editForm").sub("${path}/drama_type/save",function(data){
				layer.msg("操作成功",{icon:1});
				query();
				layer.close(index);
			},function(data){
...
var delete_action = function(){
	...
	layer.confirm('是否删除当前数据？', {
	  ...
	  title:'系统提示'
		...
		post("${path}/drama_type/delete",{id:vas[0].id},function(data){
...
var hide = function(){
	...
	layer.confirm('是否隐藏当前数据？', {
	  ...
	  title:'系统提示'
		...
		post("${path}/drama_type/hide",{id:vas[0].id},function(data){
...
var show = function(){
	...
	layer.confirm('是否显示当前数据？', {
	  ...
	  title:'系统提示'
		...
		post("${path}/drama_type/show",{id:vas[0].id},function(data){
			layer.msg('操作成功',{icon:1});
			query();
		},function(data){
			layer.alert(data.msg,{icon:2});
